package zhanglubin.legend.game.sousou.character
{
	...
	public class LSouSouCharacterS extends LSouSouCharacter
	{
		...
		public function ai():void{
			...
			/*
			this.path = LSouSouObject.sStarQuery.path(
				new LCoordinate(this.locationX,this.locationY),
				new LCoordinate(obj.nodeparent.x,obj.nodeparent.y));
			trace(this.index,"ai",this.belong,this.path);
			*/
		}
		...
		private function attackOver(event:LEvent):void{
			...
			if(this.attackIndex < this.attackNumber){
				...
				if(targetCharacter.targetCharacter && atAttackRect(targetCharacter,new LCoordinate(this.locationX,this.locationY))){
		 ...
		 * 计算攻击目标
		 */
		public function getAttTarget():Object{
			var charas:LSouSouCharacterS;
			var i:int,j:int;
			...
			//计算所有攻击范围
			var attRange:Array = getAttRange();
			...
			for each(obj in attRange){
				if(characterSArray[obj.x + "," + obj.y]){
					...
					intHert = LSouSouCalculate.getHertValue(this,charas);
					obj.hert = intHert;
		 ...
		 * 计算所有攻击范围
		 */
		private function getAttRange():Array{
			LSouSouObject.sMap.roadList = LSouSouObject.sStarQuery.makePath(LSouSouObject.charaSNow);
			...
			var node:Node;
			//兵种攻击范围
			for each ( var roundment:String in this._member.rangeAttack) {
		...
		private function getDistance(fromCoordinate:LCoordinate,toCoordinate:LCoordinate):int{
